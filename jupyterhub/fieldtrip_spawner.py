"""Class for spawning fieldtrip containers"""
...
class FieldtripSpawner(LoggingConfigurable):
    user = Any()
    hub = Any()
    authenticator = Any()
    api_token = Unicode()
    ...
    cmd = Command(['echo'], config=True,
                  help="""The command used for starting a fieldtrip server."""
                  )
    args = List(['spawning fieldtrip container'], config=True,
                help="""Extra arguments to be passed to the fieldtrip server"""
                )

    default_url = Unicode('', config=True,
                          help="""The default URL for the fieldtrip server.
    ...
    def __init__(self, **kwargs):
        super(FieldtripSpawner, self).__init__(**kwargs)
...
class FieldtripProcessSpawner(FieldtripSpawner):
    """A Docker Process Spawner that just uses Docker Fieldtrip containers for users.
    ...
    container_image = Unicode("fieldtrip", config=True)
    #TODO can we remove this arg?
    ...
    hub_ip_connect = Unicode(
        ...
        help=dedent(
            """
            If set, FieldtripProcessSpawner will configure the containers to use
    ...
    @gen.coroutine
    def get_state(self):
        state = super(FieldtripProcessSpawner, self).get_state()
    ...
    def get_env(self):
        env = super(FieldtripProcessSpawner, self).get_env()
        env.update(dict(
    ...
    def start(self,credential, image=None, extra_create_kwargs=None,
        ...
        if container is None:
            ...
            host_config = recursive_format(
               host_config,
               **extra_create_params
            )

            self.log.debug("Starting Fieldtrip host with config: %s", host_config)
    ...
    def clear_state(self):
        """clear pid state"""
        super(FieldtripProcessSpawner, self).clear_state()
        self.container_id = ''
