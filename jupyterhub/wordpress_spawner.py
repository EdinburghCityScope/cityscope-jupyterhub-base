"""Class for spawning wordpress containers"""
...
class WordpressSpawner(LoggingConfigurable):
    user = Any()
    hub = Any()
    authenticator = Any()
    api_token = Unicode()
    ...
    cmd = Command(['echo'], config=True,
                  help="""The command used for starting a wordpress server."""
                  )
    args = List(['spawning wordpress container'], config=True,
                help="""Extra arguments to be passed to the wordpress server"""
                )

    default_url = Unicode('', config=True,
                          help="""The default URL for the wordpress server.
    ...
    def __init__(self, **kwargs):
        super(WordpressSpawner, self).__init__(**kwargs)
...
class WordpressProcessSpawner(WordpressSpawner):
    """A Docker Process Spawner that just uses Docker Wordpress containers for users.
    ...
    container_image = Unicode("wordpress", config=True)
    ...
    hub_ip_connect = Unicode(
        ...
        help=dedent(
            """
            If set, WordpressProcessSpawner will configure the containers to use
    ...
    def get_env(self):
        env = super(WordpressProcessSpawner, self).get_env()
        env.update(dict(
    ...
    def clear_state(self):
        """clear pid state"""
        super(WordpressProcessSpawner, self).clear_state()
        self.container_id = ''
