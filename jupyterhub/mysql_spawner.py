"""Class for spawning mysql containers"""
...
class MySQLSpawner(LoggingConfigurable):
    user = Any()
    hub = Any()
    authenticator = Any()
    api_token = Unicode()
    ...
    cmd = Command(['echo'], config=True,
                  help="""The command used for starting a mysql server."""
                  )
    args = List(['spawning mysql container'], config=True,
                help="""Extra arguments to be passed to the mysql server"""
                )

    default_url = Unicode('', config=True,
                          help="""The default URL for the mysql server.
    ...
    def __init__(self, **kwargs):
        super(MySQLSpawner, self).__init__(**kwargs)
...
class MySQLProcessSpawner(MySQLSpawner):
    """A Docker Process Spawner that just uses Docker MySQL containers for users.
    ...
    container_image = Unicode("mysql", config=True)
    ...
    def get_env(self):
        env = super(MySQLProcessSpawner, self).get_env()
        env.update(dict(
